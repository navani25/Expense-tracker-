...
interface GroupedTransfers { [key: string]: Transfer[]; }
...
const EmptyState: React.FC<{
    ...
    switch (type) {
        case 'income':
            ...
            break;
        case 'transfer':
            icon = ( <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-violet-500 dark:text-violet-400"><path d="M16 3h5v5"/><path d="M4 20L21 3"/><path d="M21 16v5h-5"/><path d="M15 15l6 6"/></svg> );
    ...
    <div className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700">
        <div className="flex items-center space-x-4 min-w-0">
            <div className={`w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center`}>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-600 dark:text-blue-400"><line x1="12" y1="2" x2="12" y2="22"></line><line x1="22" y1="12" x2="2" y2="12"></line></svg>
            </div>
            <div className="min-w-0">
...
interface HistoryProps {
    ...
    onAdd: (type: 'expense' | 'income' | 'transfer') => void;
...
const History: React.FC<HistoryProps> = ({ expenses, income, transfers, onEditTransaction, onDeleteTransaction, onAdd, setActivePage, currency }) => {
  ...
  const onTouchEnd = () => {
    ...
    if (isLeftSwipe) { if (activeTab === 'expense') setActiveTab('income'); else if (activeTab === 'income') setActiveTab('transfer'); } 
    else if (isRightSwipe) { if (activeTab === 'transfer') setActiveTab('income'); else if (activeTab === 'income') setActiveTab('expense'); }
  ...
  const groupedTransfers = transfers.reduce((acc: GroupedTransfers, transferItem) => { const month = new Date(transferItem.date).toLocaleString('default', { month: 'long', year: 'numeric' }); if (!acc[month]) acc[month] = []; acc[month].push(transferItem); return acc; }, {} as GroupedTransfers);

  const currencySymbol = CURRENCIES.find(c => c.code === currency)?.symbol || '$';
  
  const getTransformX = () => { if (activeTab === 'income') return '-100%'; if (activeTab === 'transfer') return '-200%'; return '0%'; };
  ...
  const TransfersView = ( <div className="h-full overflow-y-auto"> {transfers.length > 0 ? ( Object.keys(groupedTransfers).map((month) => ( <div key={month} className="mb-6"> <h2 className="px-4 py-2 text-sm font-semibold text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 sticky top-0 z-[1]">{month}</h2> <div> {groupedTransfers[month].map(item => <TransferListItem key={item.id} transfer={item} currencySymbol={currencySymbol} onEdit={onEditTransaction} onDelete={onDeleteTransaction} />)} </div> </div> )) ) : ( <EmptyState type="transfer" onActionClick={() => onAdd('transfer')} /> )} </div> );

  const shouldShowFab = (activeTab === 'expense' && expenses.length > 0) || (activeTab === 'income' && income.length > 0) || (activeTab === 'transfer' && transfers.length > 0);
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 min-h-full flex flex-col">
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-900 sticky top-0 z-10 sm:mt-6">
        <div className="grid grid-cols-3 gap-2 p-1 bg-gray-200 dark:bg-gray-800 rounded-lg max-w-2xl mx-auto">
            ...
            <button onClick={() => setActiveTab('transfer')} className={`px-4 py-2 text-sm font-semibold rounded-md transition-all duration-200 ${activeTab === 'transfer' ? 'bg-white dark:bg-gray-700 text-violet-600 dark:text-violet-400 shadow' : 'text-gray-600 dark:text-gray-400'}`}>Transfer</button>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <div className="w-full flex-shrink-0 h-full">{TransfersView}</div>
        </div>
      </div>
